class GameScene3 extends Phaser.Scene {
    ...
    create(data) {
        ...
        const tileset = map3.addTilesetImage('kokomi tileset', 'tileset');
        const collectiblesNeeded = 180;

        
        

        const platformLayer = map3.createLayer('platform', tileset, 0, 0);
        platformLayer.setCollisionByProperty({ collides: true });

        const waterLayer = map3.createLayer('water', tileset);
        waterLayer.setCollisionByProperty({ collides: true });
     
        const vanishLayer = map3.createLayer('vanish', tileset);
        vanishLayer.setCollisionByProperty({ collides: true });

        const portalLayer = map3.createLayer('portal', tileset);
    ...
    if (this.score >= collectiblesNeeded) {
        ...
        const floralLayer = map3.createLayer('keys', tileset);
        const cloudsLayer = map3.createLayer('clouds', tileset);

        
        const collectiblesLayer = map3.createLayer('collectibles', tileset);
